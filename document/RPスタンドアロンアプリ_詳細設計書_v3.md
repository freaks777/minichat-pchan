# RPスタンドアロンアプリ 詳細設計書 v3.11（コア／プラグイン分離版）
...
全APIに同一オリジン確認・`Cache-Control: no-store`。reveal はPOST限定＋レート制限（30回/60秒）。
...
| `frontend/js/i18n.js` | 日英11キー追加 |
| `frontend/studio.html` / `studio.js` | Persona Studio 機密対応 |
| `tests/test_regressions.py` | 14件（+5件） |
